...
const MostPopularTvShows = (props) => (
  <TableRow>
    <TableCell>
      <Link href="/">
...
function MostPopularTvShowsList() {
  const [sortField, setSortField] = React.useState("rk")
  const [sortDirection, setSortDirection] = useState("ASC")

  const isMobile = useMediaQuery((theme) => theme.breakpoints.down("sm"))

  const url = "http://localhost:8000/mostpopulartvshows"
  ...
  const listOfShows = useMemo(() => {
    ...
    return sorted.map((show) => {
      return <MostPopularTvShows key={show.id} showList={show} />
    })
...
export default MostPopularTvShowsList
