...
const Top10Movies = (props) => (
  <TableRow>
    <TableCell>
      <Link href="/">
        <img src={props.movieList.image} alt={props.movieList.movie} width="45" height="67" />
      </Link>
    </TableCell>
    ...
    <TableCell align="left">
      <Box sx={{ display: "flex", alignItems: "center", flexWrap: "wrap" }}>
        <StarIcon style={{ color: "gold" }} /> {props.movieList.rating}
      </Box>
    </TableCell>
    <TableCell align="left">
      <IconButton>
...
function Top10MoviesList() {
  const [sortField, setSortField] = React.useState("r")
  const [sortDirection, setSortDirection] = useState("ASC")

  const isMobile = useMediaQuery((theme) => theme.breakpoints.down("sm"))

  const url = "http://localhost:8000/top10movies"

  const getTop10Movies = (url) => axios.get(url).then((res) => res.data)

  const { data, error } = useSWR(url, getTop10Movies)
  ...
  const listOfMovies = useMemo(() => {
    ...
    return sorted.map((movie) => {
      return <Top10Movies key={movie.id} movieList={movie} />
    })
              ...
              <Item sx={{ height: "100%" }}>
                <Item color="inherit" sx={{ display: "flex", flexDirection: "row" }}>
                  ...
                  <TextField
                    ...
                    size="small"
                  >
                    <MenuItem value="r">Imdb Rating</MenuItem>
                  </TextField>
                  <Tooltip
...
export default Top10MoviesList
